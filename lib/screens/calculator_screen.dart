// ══════════════════════════════════════════════════════════
//  screens/calculator_screen.dart  ✅ نسخة مُصحَّحة
//
//  الإصلاحات المطبّقة (إضافةً لما كان موجوداً):
//  ✅ #1  حذف import مكرر لـ flutter/services.dart + ignore directive
//  ✅ #2  جميع withOpacity المُهمَلة → withValues(alpha:) (23 موضعاً)
//  ──────────────────────────────────────────────────────────
//  (موجود مسبقاً ومحافَظ عليه):
//  ✅  _history منقولة من global إلى Provider محلي
//  ✅  سجل الحسابات يُمسح تلقائياً عند الخروج
...
// ✅ إصلاح: ChangeNotifier محلي بدلاً من قائمة global
//    تعيش طوال حياة CalculatorScreen فقط ثم تتحرر تلقائياً
class _CalcHistoryNotifier extends ChangeNotifier {
  final List<_CalcRecord> _records = [];

  List<_CalcRecord> get records => List.unmodifiable(_records);
  int get count => _records.length;

  void add(_CalcRecord record) {
    _records.insert(0, record);
    if (_records.length > 10) _records.removeLast();
    notifyListeners();
  }
...
class _CalculatorScreenState extends State<CalculatorScreen>
  ...
  // ✅ الـ Notifier يعيش هنا ويتحرر عند dispose تلقائياً
  final _historyNotifier = _CalcHistoryNotifier();
  ...
  void dispose() {
    _tabCtrl.dispose();
    _historyNotifier.dispose(); // ✅ تحرير الذاكرة عند الخروج
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ توفير الـ Notifier لجميع التبويبات الأبناء
    return ChangeNotifierProvider.value(
      value: _historyNotifier,
      child: Scaffold(
        body: SafeArea(
  ...
  Widget _buildHeader(BuildContext context, BannaaLocalizations t) {
    // ✅ قراءة العدد من الـ Provider بدلاً من المتغير global
    return Consumer<_CalcHistoryNotifier>(
      builder: (_, hist, __) => Padding(
...
class _ConcreteTabState extends State<_ConcreteTab> {
  ...
  void _saveToHistory(
      AppSettingsProvider s, MixRatios mix, BannaaLocalizations t) {
    if (_vol <= 0) return;
    final cost = _estimateCost(_vol, mix, s.prices);
    // ✅ استخدام Provider بدلاً من المتغير global
    context.read<_CalcHistoryNotifier>().add(_CalcRecord(
        ...
        DarkCard(
          child: Column(children: [
              ...
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Container(
                      ...
                      decoration: BoxDecoration(
                          color:
                              hasResult ? AppTheme.success : AppTheme.textMuted,
                          shape: BoxShape.circle)),
                  const SizedBox(width: 4),
                  Text(t.tr('liveResults'),
                      style: GoogleFonts.cairo(
                          fontSize: 9,
                          color:
                              hasResult ? AppTheme.success : AppTheme.textMuted,
            ...
            Row(children: [
              Expanded(
                  child: _liveInput(
                      _lCtrl, t.tr('length'), t.tr('meter'), _onChanged)),
              const SizedBox(width: 10),
              Expanded(
                  child: _liveInput(
                      _wCtrl, t.tr('width'), t.tr('meter'), _onChanged)),
            ]),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(
                  child: _liveInput(
                      _hCtrl, t.tr('heightDepth'), t.tr('meter'), _onChanged)),
              const SizedBox(width: 10),
              Expanded(
                  child: _liveInput(
                      _cCtrl, t.tr('count'), t.tr('unitCount'), _onChanged,
                      isInt: true)),
            ]),
          ]),
        ),
...
class _MaterialsTabState extends State<_MaterialsTab> {
  ...
  void _saveToHistory(
      AppSettingsProvider s, Map<String, double> res, BannaaLocalizations t) {
    // ✅ استخدام Provider بدلاً من المتغير global
    context.read<_CalcHistoryNotifier>().add(_CalcRecord(
        ...
        DarkCard(
          child:
            ...
            if (_showAdvanced) ...[
              ...
              _sliderRow(
                  context,
                  '🪣 ${t.tr('cement')} (${t.tr('kg')}/${t.tr('perM3')})',
                  cR,
                  250,
                  500,
                  (v) => setState(() => _cementOverride = v)),
              _sliderRow(context, '🏖️ ${t.tr('sand')}', sR, 0.30, 0.65,
                  (v) => setState(() => _sandOverride = v),
                  divisions: 35),
              _sliderRow(context, '🪨 ${t.tr('gravel')}', gR, 0.50, 0.90,
                  (v) => setState(() => _gravelOverride = v),
                  divisions: 40),
              ...
              TextButton(
                  ...
                  child: Text(
                      '↺ ${t.tr('resetCodeRatios')} ${s.buildingCode.short}',
                      style: GoogleFonts.cairo(
                          fontSize: 11, color: AppTheme.accent))),
            ],
          ]),
...
class _SteelTabState extends State<_SteelTab> {
  ...
  double get _weight => (_len > 0 && _count > 0)
      ? _len * _count * (_kgPerMeter[_diameter] ?? 0.888)
      : 0;
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        // ── رسم SVG حديد ───────────────────────────────
        _SteelBarSVG(
            diameter: _diameter, count: _count > 0 ? _count.clamp(1, 6) : 0),
        const SizedBox(height: 14),
        ...
        DarkCard(
          child: Column(children: [
            ...
            Row(children: [
              Expanded(
                  child: _liveInput(_lenCtrl, t.tr('barLength'), t.tr('meter'),
                      () => setState(() {}))),
              const SizedBox(width: 10),
              Expanded(
                  child: _liveInput(_countCtrl, t.tr('barCount'),
                      t.tr('barUnit'), () => setState(() {}),
                      isInt: true)),
            ]),
            const SizedBox(height: 8),
            Container(
          ...
          child: hasResult
              ? Column(children: [
                  ...
                  _LiveResultCard(
                    ...
                    costLabel: t.tr('costEstim'),
                    rows: [
                      _ResultRow('📏', t.tr('selectedDiameter'),
                          'Ø$_diameter ${t.tr('mm')}',
                          color: AppTheme.accent),
                      _ResultRow('⚖️', t.tr('barWeightPerM'),
                          ...
                          color: AppTheme.info),
                      _ResultRow(
                          '🔢', t.tr('barCount'), '$_count ${t.tr('barUnit')}',
                          color: AppTheme.textSub),
...
class _HistoryTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = BannaaLocalizations.of(context);
    // ✅ القراءة من Provider بدلاً من المتغير global
    return Consumer<_CalcHistoryNotifier>(
      builder: (_, hist, __) {
        ...
        return records.isEmpty
            ...
            : Column(children: [
                Padding(
                  ...
                  child: Row(children: [
                    Text('${t.tr('calcHistory')} (${records.length})',
                        style: GoogleFonts.cairo(
                            fontSize: 13,
                            fontWeight: FontWeight.w700,
                            color: AppTheme.textSub)),
                    ...
                    GestureDetector(
                      onTap: () {
                        hist.clear();
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                          content: Text(t.tr('calcHistoryClearedMsg'),
                              style: GoogleFonts.cairo(color: Colors.black)),
                          backgroundColor: AppTheme.accent,
                          behavior: SnackBarBehavior.floating,
                          duration: const Duration(seconds: 2),
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final rec = records[i];
                      final mins =
                          DateTime.now().difference(rec.time).inMinutes;
...
class _CubePainter extends CustomPainter {
  ...
  _CubePainter(
      {required this.l, required this.w, required this.h, required this.t});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    drawDim(bfl - const Offset(0, 6), bfr - const Offset(0, 6), wLabel,
        above: false);
    ...
    canvas.drawLine(
        bfr + const Offset(8, 0), tfr + const Offset(8, 0), dimLine);
    ...
    drawDim(tfr + const Offset(3, -3), tbr + const Offset(3, -3), lLabel,
        above: true);
    ...
    if (l > 0 && w > 0 && h > 0) {
      ...
      volTp.paint(
          canvas, Offset(cx - volTp.width / 2, cy - volTp.height / 2 - 2));
    } else {
      // وسم توضيحي
...
class _CodeBadge extends StatelessWidget {
  ...
  const _CodeBadge(
      {required this.code, required this.grade, required this.gradeLabel});
...
Widget _sliderRow(BuildContext context, String label, double val, double min,
    double max, ValueChanged<double> onChange,
    {int divisions = 20}) {
  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Text(label,
          style: GoogleFonts.cairo(fontSize: 11, color: AppTheme.textSub)),
      Text(val.toStringAsFixed(val >= 10 ? 0 : 2),
          style: GoogleFonts.cairo(
              fontSize: 11,
              color: AppTheme.accent,
              fontWeight: FontWeight.w700)),
