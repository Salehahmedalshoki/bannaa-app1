// ══════════════════════════════════════════════════════════
//  screens/report_screen.dart — مع الترجمة الكاملة
// ══════════════════════════════════════════════════════════
...
class _ReportScreenState extends State<ReportScreen> {
  ...
  void _copyToClipboard(BannaaLocalizations t, AppSettingsProvider s) {
    final text = _buildTextReport(t, s);
    Clipboard.setData(ClipboardData(text: text));
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(t.tr('reportCopied'), style: GoogleFonts.cairo(color: Colors.white)),
  ...
  String _buildTextReport(BannaaLocalizations t, AppSettingsProvider s) {
    ...
    buf.writeln('${t.tr('date')}: ${p.createdAt.day}/${p.createdAt.month}/${p.createdAt.year}');
    buf.writeln('──────────────────────────────');
    buf.writeln('${t.tr('totalConcreteVolumeTitle')}: ${p.totalVolume.toStringAsFixed(2)} ${t.tr('perM3')}');
    buf.writeln('──────────────────────────────');
    for (final m in mats) {
      buf.writeln('${m.icon} ${m.name}: ${m.quantity.toStringAsFixed(1)} ${m.unit} — ${m.totalCost.toStringAsFixed(0)} $sym');
    }
    buf.writeln('──────────────────────────────');
    buf.writeln('${t.tr('grandTotal')}: ${p.totalCost.toStringAsFixed(0)} $sym');
    buf.writeln('══════════════════════════════');
  ...
  Widget _buildReportPreview(BuildContext context, BannaaLocalizations t,
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(
          color: Colors.black.withOpacity(0.2), blurRadius: 20, offset: const Offset(0, 4))]),
      child: Column(children: [
        // رأس التقرير
        Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(t.tr('reportTitle'), style: GoogleFonts.cairo(
            ...
            Container(
              ...
              child: Text('${p.totalVolume.toStringAsFixed(1)} ${t.tr('perM3')}',
                style: GoogleFonts.cairo(
                  fontSize: 13, color: Colors.black, fontWeight: FontWeight.w800)),
            ),
          ]),
        ),
        ...
        if (_includeComponents) ...[
          Padding(
            ...
            child: Row(children: [
              Text(t.tr('materialCol'), style: _hStyle(), textAlign: TextAlign.right),
              const Spacer(),
              SizedBox(width: 70, child: Text(t.tr('quantityCol'),
                style: _hStyle(), textAlign: TextAlign.center)),
              SizedBox(width: 70, child: Text(t.tr('costCol'),
        ...
        if (_includePrices)
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(t.tr('grandTotal'), style: GoogleFonts.cairo(
        ...
        if (_includeNotes)
          Container(
            ...
            child: Text(t.tr('reportNote'),
              style: GoogleFonts.cairo(fontSize: 9, color: AppTheme.textMuted, height: 1.5),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(m.name, style: GoogleFonts.cairo(
          fontSize: 11, color: AppTheme.textPrimary, fontWeight: FontWeight.w600))),
        SizedBox(width: 90, child: Text(
          ...
          textAlign: TextAlign.center)),
        SizedBox(width: 70, child: Text('${m.totalCost.toStringAsFixed(0)}',
          style: GoogleFonts.cairo(
    ...
    return DarkCard(
      child: Column(children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text(t.tr('reportOptions'), style: GoogleFonts.cairo(
            fontSize: 13, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
        ]),
        const SizedBox(height: 12),
        _optionSwitch(t.tr('inclMaterialTable'),
          _includeComponents, (v) => setState(() => _includeComponents = v)),
        _optionSwitch(t.tr('inclPrices'),
          _includePrices, (v) => setState(() => _includePrices = v)),
        _optionSwitch(t.tr('inclNote'),
          _includeNotes, (v) => setState(() => _includeNotes = v)),
      ]),
  ...
  Widget _buildExportButtons(BannaaLocalizations t, AppSettingsProvider s) {
    return Padding(
      ...
      child: Column(children: [
        GoldenButton(label: t.tr('downloadPdf'), icon: '⬇️',
          isLoading: _isExporting, onTap: _exportPdf),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: GoldenButton(label: t.tr('copyText'), icon: '📋',
            outline: true, onTap: () => _copyToClipboard(t, s))),
          const SizedBox(width: 10),
          Expanded(child: GoldenButton(label: t.tr('shareBtn'), icon: '📤',
            outline: true, onTap: _exportPdf)),
        ]),
      ]),
    );
  }
  ...
  Widget _infoChip(String icon, String label) => Container(
    ...
    decoration: BoxDecoration(
      color: AppTheme.surfaceDark, borderRadius: BorderRadius.circular(8),
      border: Border.all(color: AppTheme.border)),
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      Text(icon, style: const TextStyle(fontSize: 11)),
      const SizedBox(width: 4),
      Text(label, style: GoogleFonts.cairo(fontSize: 10, color: AppTheme.textSub)),
    ]),
  );
