...
class _DimensionsScreenState extends State<DimensionsScreen> {
  ...
  Future<void> _calculate() async {
    ...
    await StorageService.saveProject(widget.project); // محلي للـ offline
    await FirestoreService.saveProject(widget.project); // سحابي
    if (!mounted) return;
...
class _AddComponentSheetState extends State<_AddComponentSheet> {
        ...
        child: Form(
          ...
          child: Column(
              ...
              children: [
                ...
                Row(children: [
                  Expanded(
                      child: _dimField(_lengthCtrl, t.tr('lengthDimLabel'), t)),
                  const SizedBox(width: 10),
                  Expanded(
                      child: _dimField(_widthCtrl, t.tr('widthDimLabel'), t)),
                ]),
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(
                  ...
                  const SizedBox(width: 10),
                  Expanded(
                      child: _dimField(_countCtrl, t.tr('countDimLabel'), t,
                          isInt: true)),
                ]),
                const SizedBox(height: 16),
                GoldenButton(
  ...
  Widget _dimField(
      TextEditingController ctrl, String label, BannaaLocalizations t,
      {bool isInt = false}) {
