// ══════════════════════════════════════════════════════════
//  screens/forgot_password_screen.dart  ✅ نسخة مُصحَّحة
//
//  الإصلاحات المطبّقة (إضافةً لما كان موجوداً):
//  ✅ #4  زر "إعادة الإرسال" محمي بـ _isLoading guard
//  ✅ #5  textInputAction.done + onFieldSubmitted على حقل الإيميل
//  ✅ #6  SingleChildScrollView لمنع overflow على الأجهزة الصغيرة
// ══════════════════════════════════════════════════════════
...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: SafeArea(
        // ✅ #6 — SingleChildScrollView يمنع overflow عند فتح الكيبورد
  ...
  Widget _buildSuccessState(BannaaLocalizations t) {
    ...
    return Column(children: [
      ...
      Text(t.tr('checkEmailReset'),
              style: GoogleFonts.cairo(
                  fontSize: 12, color: AppTheme.textMuted, height: 1.7),
              textAlign: TextAlign.center)
      ...
      // ✅ #4 — زر إعادة الإرسال محمي بـ _isLoading
      GoldenButton(
        label: t.tr('resendLink'),
        outline: true,
        icon: '🔄',
        onTap: _isLoading
            ? null
